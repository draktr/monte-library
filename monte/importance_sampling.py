...
def importance(importance_sampler, importance_density, target_density, n):
    ...
    :param importance_sampler: Function that samples from our importance distribution. Should be a distribution that is easy to sample from.
    ...
    :param target_density: Probability density/mass function of the distribution we want to have samples from. Intended to be a distribution that is difficult to sample from.
    :type target_density: callable
    :param n: Number of samples to be sampled.
    :type n: itn
    :return: Samples from our `target_distribution` that are obtained indirectly through importance sampling
    :rtype: ndarray
    """
